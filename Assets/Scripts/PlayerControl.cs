...
public class PlayerControl : MonoBehaviour
{
    ...
    private GearMeter gearMeter;        // Reference to the PlayerControl script.
    private bool charging;
    public float gearCooldown = 2f;
    ...
    void FixedUpdate()
    {
        //Debug.Log("Unit Speed: " + (Mathf.Abs(GetComponent<Rigidbody2D>().velocity.x)));
        // Cache the horizontal input.
        float h = Input.GetAxis("Horizontal");

        // The Speed animator parameter is set to the absolute value of the horizontal input.
        anim.SetFloat("speed", Mathf.Abs(h));
        //if ((h > 0 && facingRight && !walled) || (h < 0 && !facingRight && !walled))
        // {
        ...
        if (Mathf.Abs(GetComponent<Rigidbody2D>().velocity.x) > 4f && grounded)
        {
            charging = true;
        }
        else
        ...
        if (gearMeter.gear == 2 && gearMeter.energy >= jumpCost)
        {
            if (jump)
            {
        ...
        if (gearMeter.energy <= 0f)
        {
            if (Time.time > lastShiftTime + gearCooldown) gearMeter.ShiftGear(0);
        }
    }
